...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 13 09:43:28 2019
...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 12 23:38:25 2019
...
for name,i in zip(combinations, X_groups):
    ...
    # Classifier 
    model = LGBMClassifier(colsample_by_tree=0.6362280797062875,
                           learning_rate=0.1433661430362572,
                           max_depth=9,
                           n_estimators=33,
                           num_leaves=35,
                           reg_lambda=0.23955062740611444,
                           boosting_type='dart')
...
#      X = impy.mice(X)[0]
    ...
    for train, test in rskf.split(X, y):
        ...
        # Save trained model
        joblib.dump(model, 'lg_mods/%s_lgbm-96-%0.2f_model.pkl'%(zita, i))
...
ax.set_alpha(0.8)
ax.set_title('%s Performance (T)'%(m_name),fontsize=18)
...
for name,i in zip(combinations, X_groups):
    ...
    zita = name
    # Load trained model
    model = joblib.load('lg_mods/%s_lgbm-96-%0.2f_model.pkl'%(zita, i))
...
ax.set_alpha(0.8)
ax.set_title('%s Validation Performance On Unseen Data (T)'%(m_name),fontsize=16)
ax.set_ylabel("%", fontsize=13);
ax.set_yticks(np.arange(0, 100, step=5))
l1=log.loc['W0-1','Inde_x'] 
l2=log.loc['W0-2','Inde_x']
...
#                           reg_lambda=0.23955062740611444,
#                           boosting_type='dart')
    model = LGBMClassifier(colsample_by_tree=0.8224950562515922,
                           learning_rate=0.3868945717483617,
                           max_depth=15,
                           n_estimators=15,
                           num_leaves=13,
                           reg_lambda=0.060540396849595335,
                           boosting_type='dart')        
...
#      X = impy.mice(X)[0]
    ...
    for train, test in rskf.split(X, y):
        ...
        # Save trained model
        #joblib.dump(model, 'lg_mods/%s_lgbm-162-%0.2f_model.pkl'%(zita, i))
...
ax = log[['AUC', 'Accuracy', 'A_PR']].plot(kind='bar', width=0.8 ,figsize=(14,8), color=['dodgerblue', 'slategray', 'brown'], fontsize=13);
ax.set_alpha(0.8)
ax.set_title('%s Performance on Merged Data (T)'%(m_name),fontsize=18)
ax.set_ylabel("%", fontsize=13);
ax.set_yticks(np.arange(0, 100, step=5))
